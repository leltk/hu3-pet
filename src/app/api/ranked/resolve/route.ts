...
export async function POST(request:Request){
  ...
  await db.query("UPDATE ranked_matches SET result=$1,lp_delta=$2,xp_delta=$3,coins_delta=$4,simulation=$5 WHERE id=$6",
    [win?"WIN":"LOSS",lpDelta,xpDelta,coinsDelta,JSON.stringify(simulation),matchId]);
  ...
  return NextResponse.json({result:win?"WIN":"LOSS",win,lpDelta,xpDelta,coinsDelta,rank,simulation,pet:updated.rows[0]});
}
