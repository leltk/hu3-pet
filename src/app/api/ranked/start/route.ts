...
export async function POST(request: Request){
 ...
 const now=new Date();
 if(!TEST_MODE&&pet.next_ranked_at&&new Date(pet.next_ranked_at)>now)return NextResponse.json({error:"O pet ainda está em partida",nextRankedAt:pet.next_ranked_at},{status:409});
 const finishes=new Date(now.getTime()+GAME_TIMERS.rankedMs);
